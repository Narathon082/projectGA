# รายงานโครงสร้างและการทำงานของส่วนส่งข้อมูลเฉลี่ยสะสมไปยัง Firebase (ไฟล์ PnOmppt.ino)

เอกสารฉบับนี้อธิบายรายละเอียดซอร์สโค้ดในไฟล์ [PnOmppt.ino](file:///c:/ProjectFlutter/projectGA/hardware/PnOmppt/PnOmppt.ino) เฉพาะส่วนที่เกี่ยวข้องกับการเชื่อมต่อเครือข่าย Wi-Fi, NTP (Network Time Protocol) และอัลกอริทึมการคำนวณ **ค่าเฉลี่ยสะสมรายชั่วโมง (Running Average)** ของกำลังไฟฟ้า (วัตต์) และกระแสไฟฟ้า (แอมป์) แล้วส่งขึ้นไปยัง Firebase Realtime Database ของระบบควบคุม MPPT แบบ P&O (Perturb and Observe)
...
1. **เก็บตัวอย่างข้อมูลทุกๆ 15 วินาที:** อ่านค่ากำลังวัตต์จากการหาจุด MPPT ปัจจุบัน (`currentPower`) และกระแสไฟฟ้า (`currentPower / vBat`)
...
## 4. โครงสร้างเวลาและการนับลูปหลัก (Execution Loop Breakdown)

กระบวนการประมวลผลหลักของ P&O MPPT จะอัปเดต PWM ทุก ๆ 1 วินาที (ค่าตั้งต้นของ `stepDelayMs = 1000ms` ซึ่งสามารถปรับผ่าน Serial ได้) การคำนวณ 15 วินาทีของตัวสะสมเฉลี่ยสะสมจึงเช็คค่าผ่านคำสั่ง `millis()` แบบ **Non-blocking Throttling** ทำงานร่วมกับลูปหลักดังนี้:

```cpp
void loop() {
  // ... [การควบคุมการอ่านค่าคำสั่ง Serial และลูป P&O] ...
  if (isRunning) {
    loopCounter++; 
    ...
    if (loopCounter >= loopsPerStep) {
      ...
      // ... การปรับสัญญาณ Duty Cycle และเขียนค่าควบคุม ...
      ...
      if (millis() - lastSampleTime >= 15000) {
        ...
        float currentAmp = currentPower / vBat; // แปลงกระแสไฟฟ้า (วัตต์ / แรงดันแบตเตอรี่ 18.0V)
...
## 5. เปรียบเทียบกับซอร์สโค้ด Gamppt.ino
...
```mermaid
sequenceDiagram
    participant ESP32 as ESP32 (P&O Main Loop)
    ...
    loop ทุกๆ 1 วินาที (การทำงานแต่ละก้าวของ P&O)
        ESP32->>ESP32: ตรวจวัดกำลังวัตต์ด้วย getPower() และปรับทิศทาง Duty Cycle
    end
